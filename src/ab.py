...
df = pd.read_csv(
    '/Users/hridayc/Developer/box_office_busters/data/super_regression/combined.csv')
...
for index, row in tqdm(df.iterrows(), total=len(df), desc="Getting actor birthdays"):
    actor_ids = [row['actor_0_id'], row['actor_1_id'],
                 row['actor_2_id'], row['actor_3_id'], row['actor_4_id']]
